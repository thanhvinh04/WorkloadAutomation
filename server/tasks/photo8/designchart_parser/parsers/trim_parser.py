...
from ..config import FIELD_LABELS, TRIM_COLUMNS
...
def parse_trim_table(df_table: pd.DataFrame, meta: TableMeta) -> list[dict[str, Any]]:
    ...
    for col, position in position_cols:
        ...
        name = _pick_first(FIELD_LABELS["name_trim"], col)
        type_val = _pick_first(FIELD_LABELS["type"], col)
        size = _pick_first(FIELD_LABELS["size"], col)
...
def rows_to_trim_df(rows: list[dict[str, Any]]) -> pd.DataFrame:
    df = pd.DataFrame(rows)
    return ensure_dataframe_columns(df, TRIM_COLUMNS)